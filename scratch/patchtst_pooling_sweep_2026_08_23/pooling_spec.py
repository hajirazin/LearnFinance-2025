"""Frozen domain configuration for the PatchTST pooling-head sweep."""
...
POOLING_HEADS = {
    "mean": PoolingHead("mean", "mean"),
    "flatten": PoolingHead("flatten", None),
}
...
def hf_config_for_head(head: PoolingHead) -> PatchTSTConfig:
    """Construct the fully explicit frozen 10/5 close-only configuration."""
...
def build_patchtst_model(head: PoolingHead) -> PatchTSTForPrediction:
    """Build one uninitialized research model."""
    return PatchTSTForPrediction(hf_config_for_head(head))
...
def frozen_configuration_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "patch_length": PATCH_LENGTH,
        "patch_stride": PATCH_STRIDE,
        "patch_count": patch_count(CONTEXT_LENGTH, PATCH_LENGTH, PATCH_STRIDE),
        "input": "one-channel adjusted-close log returns",
        "seeds": list(SEEDS),
        "pooling_heads": {
            name: {
                **asdict(head),
                "effective_hf_config": hf_config_for_head(head).to_dict(),
            }
            for name, head in POOLING_HEADS.items()
        },
