#!/usr/bin/env python3
"""Run the frozen full-universe close-only PatchTST pooling-head sweep."""
...
from pooling_panel import build_fold_panel, panel_arrays, panel_identity
...
def confirmatory_unlock_payload(
    ...
    *,
    ...
    """Verify all six confirmatory checkpoints before permitting label access."""
    prefix = "confirmatory_2026/"
    checkpoints = {
        key: value for key, value in training_metadata.items() if key.startswith(prefix)
    }
    if len(checkpoints) != 6:
        raise RuntimeError("confirmatory unlock requires exactly 6 checkpoints")
    expected = {
        f"confirmatory_2026/{pooling_name}/{seed}"
...
def _checkpoint_model(job: TrainingJob) -> torch.nn.Module:
    path = MODEL_DIR / job.fold_name / job.pooling_name / str(job.seed) / "weights.pt"
    model = __import__("pooling_spec").build_patchtst_model(
        POOLING_HEADS[job.pooling_name]
    )
...
def _run_one_fold(
    ...
    *,
    ...
    seed_frames: dict[str, list[pd.DataFrame]] = {name: [] for name in POOLING_HEADS}

    for pooling_name, head in POOLING_HEADS.items():
        for seed in SEEDS:
            job_tuple = (fold_name, pooling_name, seed)
            ledger.begin(job_tuple)
            model: torch.nn.Module | None = None
            try:
                print(
                    f"TRAIN fold={fold_name} pooling={pooling_name} "
                    f"seed={seed} device={device}",
                    flush=True,
                )
                model_dir = MODEL_DIR / fold_name / pooling_name / str(seed)
                model, metadata = train_pooling_seed(
                    fold,
                    head,
                    seed,
                    train_x,
                ...
                training_metadata[f"{fold_name}/{pooling_name}/{seed}"] = metadata
                if not confirmatory:
                    ...
                    frames.append(frame)
                    seed_frames[pooling_name].append(frame)
            finally:
    ...
    if confirmatory:
        ...
        for pooling_name in POOLING_HEADS:
            for seed in SEEDS:
                job = TrainingJob(fold_name, pooling_name, seed)
                job_tuple = tuple(job)
                ledger.begin(job_tuple)
                model = None
                try:
    ...
    for pooling_name, values in seed_frames.items():
        frames.append(_ensemble(values, pooling_name))
...
def _comparisons(frames: list[pd.DataFrame]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for pooling_name in POOLING_HEADS:
        arm_gate[pooling_name] = {}
        for evidence_kind in ("development", "confirmatory"):
            challenger = _evidence_frame(frames, pooling_name, evidence_kind)
            arm_gate[pooling_name][evidence_kind] = {
    ...
    for evidence_kind in ("development", "confirmatory"):
        ...
        for challenger_name, reference_name in combinations(POOLING_HEADS, 2):
    ...
    *,
    ...
    try:
        with TrainingRunLock(LOCK_PATH):
            ...
            for pooling_name in POOLING_HEADS:
                fold_metrics = {
                    fold_name: metrics[f"{fold_name}/{pooling_name}/ensemble"]
                    for fold_name in EVALUATION_FOLDS
                }
                clearance[pooling_name] = research_clearance(
                    fold_metrics, arm_gate[pooling_name]
                )
            ...
            manifest = {
                ...
                **research_side_effects_manifest(),
                "limitations": [
                    ...
                    "The 2024-2025 development folds were previously opened by the geometry sweep; they remain development evidence here.",
                    "The mean 10/5 confirmatory-2026 numbers were already observed in the geometry sweep. 2026 is confirmatory for the pooling hypothesis only because flatten is not selected on 2026.",
