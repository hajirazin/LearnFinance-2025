"""Exclusive deterministic training for one geometry/fold/seed at a time."""
...
def training_jobs() -> tuple[TrainingJob, ...]:
    ...
    return tuple(
        TrainingJob(fold_name, geometry_name, seed)
...
class TrainingRunLock(AbstractContextManager["TrainingRunLock"]):
    ...
    def __enter__(self) -> TrainingRunLock:
        ...
        try:
            ...
            self._handle = None
            raise RuntimeError(
                "a geometry training runner is already active"
            ) from error
        self._handle.seek(0)
...
def training_fingerprint(
    ...
    *,
    ...
    digest.update(json.dumps(asdict(geometry), sort_keys=True).encode())
    ...
    *,
    ...
    if weights_path.exists() and metadata_path.exists():
        ...
        if metadata.get("fingerprint") == fingerprint and metadata.get(
            ...
            restored = build_patchtst_model(geometry)
    ...
    model = build_patchtst_model(geometry).to(device)
        ...
        for batch_x, batch_y in loader:
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(
                    f"nonfinite loss for {fold.name}/{geometry.name}/{seed}"
                )
            loss.backward()
    ...
    if best_state is None:
        raise RuntimeError(f"no checkpoint for {fold.name}/{geometry.name}/{seed}")
...
def run_mps_determinism_smoke() -> dict[str, Any]:
    ...
    geometry = PATCH_GEOMETRIES["patch_8_stride_4"]
