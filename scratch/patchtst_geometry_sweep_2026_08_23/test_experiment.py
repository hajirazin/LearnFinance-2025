"""Regression tests for the isolated PatchTST patch-geometry sweep."""
...
from geometry_panel import build_fold_panel, panel_arrays
...
def test_patch_counts_are_14_11_6() -> None:
    assert {
        name: patch_count(CONTEXT_LENGTH, geometry.patch_length, geometry.patch_stride)
        for name, geometry in PATCH_GEOMETRIES.items()
    } == {
        "patch_8_stride_4": 14,
        "patch_10_stride_5": 11,
        "patch_16_stride_8": 6,
    }


def test_patch_geometry_is_the_only_arm_difference() -> None:
    configs = [
        hf_config_for_geometry(geometry) for geometry in PATCH_GEOMETRIES.values()
    ]
    normalized = []
    for config in configs:
        values = config.to_dict()
        values.pop("patch_length")
        values.pop("patch_stride")
        normalized.append(values)

    assert normalized[1:] == normalized[:-1]
    assert all(config.num_input_channels == 1 for config in configs)
    assert all(config.pooling_type == "mean" for config in configs)
...
def test_training_fingerprint_changes_for_fold_geometry_seed_or_data() -> None:
    ...
    base = training_fingerprint(
        ...
        PATCH_GEOMETRIES["patch_8_stride_4"],
        20260823,
    ...
    variants = [
        ...
        training_fingerprint(
            EVALUATION_FOLDS["development_2024"],
            PATCH_GEOMETRIES["patch_10_stride_5"],
            20260823,
            (train_x, train_y, validation_x, validation_y),
            max_epochs=60,
            patience=8,
...
def test_training_job_order_is_strictly_fold_arm_seed() -> None:
    ...
    assert len(jobs) == 27
    assert len({(job.fold_name, job.geometry_name, job.seed) for job in jobs}) == 27
    assert jobs[0] == ("development_2024", "patch_8_stride_4", 20260823)
    assert jobs[1] == ("development_2024", "patch_8_stride_4", 20260824)
    assert jobs[-1] == ("confirmatory_2026", "patch_16_stride_8", 20260825)
...
def test_confirmatory_unlock_requires_all_nine_checkpoint_hashes(
    tmp_path: Path,
) -> None:
    training: dict[str, dict[str, object]] = {}
    for geometry in PATCH_GEOMETRIES:
        for seed in SEEDS:
            path = tmp_path / geometry / str(seed) / "weights.pt"
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(f"{geometry}/{seed}".encode())
            training[f"confirmatory_2026/{geometry}/{seed}"] = {
...
def test_runner_never_has_more_than_one_live_model() -> None:
    ...
    first = ("development_2024", "patch_8_stride_4", 20260823)
    second = ("development_2024", "patch_8_stride_4", 20260824)
