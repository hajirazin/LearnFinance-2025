#!/usr/bin/env python3
"""E6: Z-scored percentage returns (E4 stationarity + E5 optimizer scale).

1. Close -> daily simple returns (0.02 == +2%)
2. Per sample: mu, sigma from the trailing 60d *return* context
3. X = (ctx_r - mu) / sigma ; y = (next5_r - mu) / sigma
4. Predict 5 z-scored returns; HF RevIN OFF (pre-zscored)
5. Eval: denorm pred_r = pred_z * sigma + mu ; score daily + weekly
   compound vs actual (Alpha-HRP-style ranking metric)

Baseline weekly_pp JSON under results/ left untouched.
Artifacts: scratch/.../exp_e6_zscore_pct_returns/
"""
...
OUT_DIR = BASE_DIR / "exp_e6_zscore_pct_returns"
...
def _close_simple_frac(price_df: pd.DataFrame) -> pd.Series:
    return price_df["close"].astype(float).pct_change()
        ...
        while i < n:
            ...
            if j - i >= config.min_week_days:
                ...
                if t >= config.context_length - 1 and t + 5 < n:
                    ctx = rets.iloc[t - config.context_length + 1 : t + 1].values
                    fut = rets.iloc[t + 1 : t + 6].values
...
def _train(
    ...
    meta = {
        "name": name,
        "representation": "zscore_simple_return_fraction",
        "example": "2% -> 0.02 -> z via context mu/sigma",
        "zscore": "per_sample_context_mean_std_on_returns",
        "num_input_channels": 1,
        "scaling": None,
        "lr": LR,
...
def _compound_week(daily_fracs: np.ndarray) -> float:
    return float(np.prod(1.0 + daily_fracs) - 1.0)
...
def _eval_walkforward(
    ...
    for week_days in _iter_weeks(prices, symbols):
        ...
        for sym in symbols:
            ...
            if not ok or len(actuals) != 5:
                continue
            fut = np.array(actuals, dtype=np.float64)
            ctx_z, fut_z, mu, sigma = _zscore(ctx, fut)
            pred_z = _predict_5_z(model, ctx_z, device)
            pred_frac = pred_z * sigma + mu  # denorm to true % fraction

            ctx_end = hist.index[-1].date().isoformat()
            for i in range(5):
                a, p = float(fut[i]), float(pred_frac[i])
                rows.append(
                    DayRow(
                        ...
                        actual_frac=a,
                        pred_frac=p,
                        actual_z=float(fut_z[i]),
                        pred_z=float(pred_z[i]),
                        abs_err_frac=abs(p - a),
            ...
            a_week = _compound_week(fut)
            p_week = _compound_week(pred_frac)
            weekly.append(
                {
                    "week_start": week_start,
                    "symbol": sym,
                    "actual_weekly_frac": a_week,
                    "pred_weekly_frac": p_week,
                    "abs_err": abs(p_week - a_week),
                    "dir_correct": bool(
                        (np.sign(p_week) == np.sign(a_week))
                        if a_week != 0
                        else (p_week == 0)
                    ),
                }
    ...
    wa = np.array([w["actual_weekly_frac"] for w in weekly])
    wp = np.array([w["pred_weekly_frac"] for w in weekly])
    weekly_mae = float(np.mean(np.abs(wp - wa)))
    weekly_naive0 = float(np.mean(np.abs(wa)))
    weekly_dir = float(np.mean([w["dir_correct"] for w in weekly]))
    ...
    summary = {
        ...
        "weekly_dir_acc": weekly_dir,
        "weekly_corr": float(np.corrcoef(wp, wa)[0, 1]) if len(wp) > 1 else None,
        "pred_frac_min": float(np.min(pred)),
        "pred_frac_max": float(np.max(pred)),
        **by_day,
    }
    return rows, summary, weekly
...
def main() -> int:
    ...
    name = "E6_zscore_pct_returns"
    model, meta = _train(name, X, y, config)
    rows, wf, weekly = _eval_walkforward(model, prices, config, symbols)
        ...
        samples.append(
            {
                ...
                "symbol": sym,
                "ctx_mu": round(sub[0].ctx_mu, 6),
                "ctx_sigma": round(sub[0].ctx_sigma, 6),
                "days": [
                    {
                        "day_i": r.day_i,
                        "date": r.date,
                        ...
                        "actual_z": round(r.actual_z, 4),
                        "pred_z": round(r.pred_z, 4),
                    }
    ...
    e4 = _load_json(BASE_DIR / "exp_e4_daily_frac" / "results" / "summary.json")
    e5 = _load_json(BASE_DIR / "exp_e5_zscore_close_price" / "results" / "summary.json")
    prior = _load_json(BASE_DIR / "results" / "summary.json")

    compare = {
        "e6": {
            "best_epoch": meta["best_epoch"],
            "mae_frac": wf["mae_frac"],
            "dir_acc": wf["dir_acc"],
            "std_pred_frac": wf["std_pred_frac"],
            "beats_naive0_daily": wf["beats_naive0_daily"],
            "weekly_mae_pp": wf["weekly_mae_pp_display"],
            "beats_naive0_weekly": wf["beats_naive0_weekly"],
            "weekly_dir_acc": wf["weekly_dir_acc"],
        }
    }
    if e4:
        compare["e4"] = {
            "best_epoch": e4["train"]["best_epoch"],
            "mae_frac": e4["walkforward"]["mae_frac"],
            "dir_acc": e4["walkforward"]["dir_acc"],
            "std_pred_frac": e4["walkforward"]["std_pred_frac"],
            "beats_naive0_daily": e4["walkforward"]["beats_naive0"],
        }
        compare["e6_better_daily_mae_than_e4"] = (
            wf["mae_frac"] < e4["walkforward"]["mae_frac"]
        )
    if e5:
        compare["e5"] = {
            "best_epoch": e5["train"]["best_epoch"],
            "mae_frac": e5["walkforward"]["mae_frac"],
            "dir_acc": e5["walkforward"]["dir_acc"],
            "weekly_mae_pp": e5["walkforward"].get("weekly_mae_pp_display"),
            "beats_naive0_weekly": e5["walkforward"].get("beats_naive0_weekly"),
        }
        compare["e6_better_daily_mae_than_e5"] = (
            wf["mae_frac"] < e5["walkforward"]["mae_frac"]
        )
    if prior:
        compare["prior_close_only_weekly_mae_pp"] = prior.get("mae_close_only_pct")
        compare["naive0_weekly_from_prior_approx_pp"] = 4.8567
    ...
    out = {
        ...
        "note": (
            "Z-score daily simple-return fractions using per-sample 60d context "
            "mu/sigma; denorm for % and weekly compound (Alpha-HRP metric)."
        ),
    }
    ...
    (RESULTS_DIR / "weekly_rows.json").write_text(json.dumps(weekly, indent=2) + "\n")

    print("\n======== E6 SUMMARY ========")
    print(json.dumps({"train": meta, "walkforward": wf, "compare": compare}, indent=2))
