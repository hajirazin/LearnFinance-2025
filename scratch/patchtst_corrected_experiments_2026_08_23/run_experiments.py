...
from evaluation import aggregate_metrics, bootstrap_intervals, paired_bootstrap_delta, weekly_statistics
...
def _require_clean_tracked_baseline() -> None:
    ...
    if _git_head() != BASELINE_COMMIT:
        raise RuntimeError(f"HEAD differs from preregistered baseline {BASELINE_COMMIT}")
    for args in (["git", "diff", "--quiet", BASELINE_COMMIT, "--"], ["git", "diff", "--cached", "--quiet", BASELINE_COMMIT, "--"]):
        if subprocess.run(args, cwd=repository, check=False).returncode != 0:
            raise RuntimeError("tracked worktree/index differs from the preregistered baseline")
...
def _load_or_download_prices() -> tuple[dict[str, pd.DataFrame], dict[str, Any]]:
    ...
    if cache_manifest.exists():
        ...
        if meta.get("requested_symbols") != SYMBOLS or not set(meta.get("files", {})).issubset(SYMBOLS):
            raise RuntimeError("adjusted cache does not match the frozen diagnostic universe")
        referenced_files = [info.get("file") for info in meta.get("files", {}).values()]
        if len(referenced_files) != len(set(referenced_files)):
            raise RuntimeError("adjusted cache manifest aliases multiple symbols to one file")
    ...
    if len(prices) < 10:
        raise RuntimeError(f"adjusted download left fewer than 10 symbols: {sorted(prices)}")
    files: dict[str, Any] = {}
...
def _prediction_frame(panel: pd.DataFrame, split: str, values: np.ndarray, arm: str, seed: str | int) -> pd.DataFrame:
    rows = panel[panel["split"] == split][["decision_date", "symbol", "actual_weekly_return"]].copy()
    ...
    rows["actual_weekly_return"] = np.expm1(rows["actual_weekly_return"].to_numpy(float))
...
def _ridge_for_validation(panel: pd.DataFrame) -> np.ndarray:
    ...
    model.fit(np.stack(train["ridge_features"]), train["actual_weekly_return"].to_numpy())
...
def _select_with_declared_tie_break(scores: dict[str, float], order: list[str]) -> str:
    if set(scores) != set(order):
        raise ValueError("selection candidates differ from the declared tie-break order")
    best = max(scores.values())
    return next(name for name in order if math.isclose(scores[name], best, rel_tol=0.0, abs_tol=1e-12))
...
def _record_failure(
    ...
    *,
    ...
    history.append({"attempt_id": attempt_id, "recorded_at_utc": datetime.now(timezone.utc).isoformat(), **failure})
...
def _train_or_resume(
    ...
    *,
    ...
    fingerprint = training_fingerprint(architecture, objective, (train_x, train_y, val_x, val_y))
    if (model_dir / "weights.pt").exists() and (model_dir / "meta.json").exists():
        try:
            model, meta = load_model_artifact(architecture, model_dir)
            if meta.get("seed") == seed and meta.get("objective") == objective and meta.get("training_fingerprint") == fingerprint:
            ...
            print(f"RETRAIN invalid artifact {objective} {architecture.name} seed={seed}: {exc}")
    return train_model(
        architecture,
        seed,
        train_x,
...
def run() -> int:
    ...
    failure_history = json.loads(failure_history_path.read_text()) if failure_history_path.exists() else []
    print("=== corrected PatchTST experiment suite ===")
    print(f"attempt_id={attempt_id}")
    _require_clean_tracked_baseline()
    json_dump(RESULTS / "summary.json", {"status": "in_progress", "attempt_id": attempt_id, "started_at_utc": datetime.now(timezone.utc).isoformat()})
    ...
    config = {
        ...
        "seeds": SEEDS,
        "splits": {name: [str(start), str(end)] for name, (start, end) in SPLITS.items()},
        "architecture": architecture_manifest(),
        "training": {"context": 60, "horizon": 5, "patch": 16, "d_model": 64, "heads": 4, "layers": 2, "ffn": 128, "learning_rate": 3e-4, "weight_decay": 0.0, "max_epochs": 60, "patience": 8},
        ...
        "bootstrap": {"unit": "decision_week", "moving_block_weeks": 4, "repetitions": 2000},
        "data": provenance,
        "dependencies": {"python": sys.version.split()[0], "torch": torch.__version__, "transformers": transformers.__version__, "pandas": pd.__version__, "numpy": np.__version__},
    }
    ...
    if not test_locked["actual_weekly_return"].isna().all() or not test_locked["y"].isna().all():
    ...
    manifest = {
        ...
        "week_counts": {name: int(locked[locked["split"] == name]["decision_date"].nunique()) for name in SPLITS},
        "common_symbols": sorted(locked["symbol"].unique()),
        "exclusions": locked.attrs.get("exclusions", {}),
        "test_labels_locked": True,
        "source_sha256": {path.name: sha256_file(path) for path in [BASE / "run_experiments.py", BASE / "experiment_core.py", BASE / "evaluation.py", BASE / "test_experiments.py"]},
    }
    ...
    for name, architecture in ARCHITECTURES.items():
        ...
        for seed in SEEDS:
            ...
            try:
                ...
                frame = _prediction_frame(locked, "validation", predict(model, val_x), name, seed)
                validation_frames[name].append(frame)
                validation_seed_metrics[name][str(seed)] = {"training": meta, "metrics": aggregate_metrics(frame)}
            except Exception as exc:
                _record_failure(
                    {"arm": name, "seed": str(seed), "stage": "daily_mse", "error": repr(exc)},
                    attempt_id=attempt_id,
                    current=failures,
    ...
    channel_candidates = ["canonical_close_only", "canonical_independent_5ch", "canonical_mixing_5ch"]
    validation_ensembles = {name: _ensemble(validation_frames[name], name, "validation") for name in ARCHITECTURES}
    architecture_validation_rank_ic = {
        name: float(np.mean([validation_seed_metrics[name][str(seed)]["metrics"]["weekly_rank_ic"] for seed in SEEDS]))
        for name in channel_candidates
    }
    if not all(np.isfinite(list(architecture_validation_rank_ic.values()))):
        raise RuntimeError(f"non-finite architecture selection score: {architecture_validation_rank_ic}")
    selected_architecture = _select_with_declared_tie_break(architecture_validation_rank_ic, channel_candidates)
    ...
    for seed in SEEDS:
        ...
        try:
            ...
            frame = _prediction_frame(locked, "validation", predict(model, val_x), listnet_name, seed)
            validation_frames[listnet_name].append(frame)
            validation_seed_metrics[listnet_name][str(seed)] = {"training": meta, "metrics": aggregate_metrics(frame)}
        except Exception as exc:
                _record_failure(
                    {"arm": listnet_name, "seed": str(seed), "stage": "listnet", "error": repr(exc)},
                    attempt_id=attempt_id,
                    current=failures,
                    history=failure_history,
                )
                raise
    validation_ensembles[listnet_name] = _ensemble(validation_frames[listnet_name], listnet_name, "validation")
    ...
    if not all(np.isfinite(list(objective_validation_rank_ic.values()))):
        raise RuntimeError(f"non-finite objective selection score: {objective_validation_rank_ic}")
    selected_objective_arm = _select_with_declared_tie_break(objective_validation_rank_ic, objective_candidates)

    control_panel_locked = control_predictions(locked)
    validation_controls: dict[str, pd.DataFrame] = {}
    for name in ["zero_return", "historical_mean", "majority_sign", "persistence_1w", "reversal_1w", "momentum_4w"]:
        validation_controls[name] = _prediction_frame(control_panel_locked, "validation", control_panel_locked.loc[control_panel_locked["split"] == "validation", name].to_numpy(), name, "control")
    validation_controls["ridge"] = _prediction_frame(locked, "validation", _ridge_for_validation(locked), "ridge", "control")
    ...
    control_order = ["zero_return", "historical_mean", "persistence_1w", "reversal_1w", "momentum_4w", "ridge"]
    strongest_control = _select_with_declared_tie_break(finite_control_scores, [name for name in control_order if name in finite_control_scores])
    ...
    for name in ["canonical_independent_5ch", "canonical_mixing_5ch"]:
        ...
        sensitivity[name] = {}
        for seed in SEEDS:
            sensitivity[name][str(seed)] = model_sensitivity(trained[(name, seed, "daily_mse")], x, close_idx=3)
    if any(value["forecast_max_abs_delta"] != 0 or value["non_close_grad_l1"] != 0 for value in sensitivity["canonical_independent_5ch"].values()):
        raise RuntimeError("channel-independent negative control was sensitive to non-Close inputs")
    if any(value["forecast_max_abs_delta"] <= 0 or value["non_close_grad_l1"] <= 0 for value in sensitivity["canonical_mixing_5ch"].values()):
        raise RuntimeError("channel-mixing arm did not use non-Close inputs")
    if not all(np.isfinite(list(value.values())).all() for arm in sensitivity.values() for value in arm.values()):
    ...
    selection = {
        ...
        "control_validation_rank_ic": {name: aggregate_metrics(frame)["weekly_rank_ic"] for name, frame in validation_controls.items()},
    ...
    pd.concat([*sum(validation_frames.values(), []), *validation_controls.values()], ignore_index=True).to_csv(RESULTS / "validation_predictions.csv", index=False)
    unlock_receipt = {
        "config_sha256": manifest["config_sha256"],
        "source_sha256": manifest["source_sha256"],
        "selection_sha256": sha256_file(RESULTS / "selection.json"),
        "validation_predictions_sha256": sha256_file(RESULTS / "validation_predictions.csv"),
    }
    ...
    locked_keys = test_locked[["decision_date", "symbol"]].astype(str).to_records(index=False).tolist()
    test = panel[panel["split"] == "test"]
    open_keys = test[["decision_date", "symbol"]].astype(str).to_records(index=False).tolist()
    ...
    for name, architecture in ARCHITECTURES.items():
        ...
        seed_results[name] = {}
        for seed in SEEDS:
            frame = _prediction_frame(panel, "test", predict(trained[(name, seed, "daily_mse")], test_x), name, seed)
            frames.append(frame)
    ...
    for seed in SEEDS:
        frame = _prediction_frame(panel, "test", predict(trained[(selected_architecture, seed, "listnet")], test_x), listnet_name, seed)
        frames.append(frame)
        prediction_frames.append(frame)
    ...
    for name in ["zero_return", "historical_mean", "majority_sign", "persistence_1w", "reversal_1w", "momentum_4w"]:
        ...
        control_frames[name] = _prediction_frame(controls, "test", values, name, "control")
    control_frames["ridge"] = _prediction_frame(panel, "test", ridge_predictions(panel), "ridge", "control")
    ...
    all_predictions.to_csv(RESULTS / "predictions.csv", index=False, float_format="%.12g")
    ...
    pd.concat(weekly_outputs, ignore_index=True).to_csv(RESULTS / "weekly_metrics.csv", index=False, float_format="%.12g")

    daily_selected = aggregate_frames[selected_architecture]
    validation_selected = aggregate_frames[selected_objective_arm]
    comparisons = {
        "adapter_stride_fix_vs_legacy": paired_bootstrap_delta(aggregate_frames["stride_only_fixed"], aggregate_frames["legacy_effective"]),
        "adapter_flatten_head_vs_stride_fix": paired_bootstrap_delta(aggregate_frames["canonical_independent_5ch"], aggregate_frames["stride_only_fixed"]),
        "adapter_canonical_vs_legacy": paired_bootstrap_delta(aggregate_frames["canonical_independent_5ch"], aggregate_frames["legacy_effective"]),
        "channel_mixing_vs_independent": paired_bootstrap_delta(aggregate_frames["canonical_mixing_5ch"], aggregate_frames["canonical_independent_5ch"]),
        "listnet_vs_daily_mse": paired_bootstrap_delta(aggregate_frames[listnet_name], daily_selected),
        "selected_vs_strongest_control": paired_bootstrap_delta(validation_selected, aggregate_frames[strongest_control]),
    }
    verdicts = {}
    for name, comparison in comparisons.items():
        rank_interval = comparison["weekly_rank_ic"]["ci95"]
        top3_interval = comparison["top3_excess"]["ci95"]
        verdicts[name] = "supported on both primary metrics" if rank_interval[0] > 0 and top3_interval[0] > 0 else "no demonstrated improvement"
    ...
    advantage_ci = comparisons["selected_vs_strongest_control"]["weekly_rank_ic"]["ci95"]
    advantage_top3_ci = comparisons["selected_vs_strongest_control"]["top3_excess"]["ci95"]
    final_gate = (
        "demonstrated screening signal"
        if selected_ci[0] > 0 and selected_top3_ci[0] > 0 and advantage_ci[0] > 0 and advantage_top3_ci[0] > 0
    ...
    for name, result in aggregate_results.items():
        metric_rows.append({"arm": name, **result["metrics"], "rank_ic_ci_low": result["ci95"]["weekly_rank_ic"][0], "rank_ic_ci_high": result["ci95"]["weekly_rank_ic"][1], "top3_excess_ci_low": result["ci95"]["top3_excess"][0], "top3_excess_ci_high": result["ci95"]["top3_excess"][1]})
    pd.DataFrame(metric_rows).to_csv(RESULTS / "metrics.csv", index=False, float_format="%.12g")

    active_model_directories = {
        MODELS / name / str(seed)
    ...
    manifest["stale_model_artifacts_excluded"] = sorted(
        str(path.relative_to(BASE)) for path in all_model_files - set(active_model_files)
    )
    ...
    manifest["run_log_note"] = "run.log is append-only; prior failed/review attempts remain visible and are not part of the final-run integrity seal"
    json_dump(RESULTS / "manifest.json", manifest)
    ...
    summary = {
        ...
        "manifest": manifest,
        "selection": {key: value for key, value in selection.items() if key != "validation_seed_results"},
        "seed_level_test_results": seed_results,
        "aggregate_test_results": aggregate_results,
        "arm_configs": {
            **architecture_manifest(),
            listnet_name: {**architecture_manifest()[selected_architecture], "objective": "listnet"},
        },
        ...
        "artifacts": {"predictions": "results/predictions.csv", "weekly_metrics": "results/weekly_metrics.csv", "metrics": "results/metrics.csv", "selection": "results/selection.json", "unlock_receipt": "results/first_test_unlock.json", "integrity": "results/integrity.json"},
    }
    ...
    json_dump(
        ...
        {
            "attempt_id": attempt_id,
            "artifacts_sha256": {str(path.relative_to(BASE)): sha256_file(path) for path in integrity_paths},
        },
    )
    print(json.dumps({"selected_architecture": selected_architecture, "selected_objective_arm": selected_objective_arm, "strongest_control": strongest_control, "verdicts": verdicts, "final_gate": final_gate, "metrics": {name: value["metrics"] for name, value in aggregate_results.items()}}, indent=2))
    return 0
...
def main() -> int:
    ...
    with lock_path.open("a+") as lock_handle:
        try:
            ...
            raise RuntimeError("another corrected PatchTST suite process owns the experiment directory") from exc
        with LOG.open("a", buffering=1) as log_handle:
            ...
            with contextlib.redirect_stdout(tee_out), contextlib.redirect_stderr(tee_err):
                ...
                try:
                    ...
                    existing = json.loads(failure_path.read_text()) if failure_path.exists() else []
                    ...
                    history.append({"attempt_id": "suite-start-failure", "recorded_at_utc": datetime.now(timezone.utc).isoformat(), **suite_failure})
