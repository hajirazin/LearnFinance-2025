...
| `POST /train/ppo-discovery/promote` | Manual promotion to `current`. **Intended gates (behavioral rule 5): only (1) test CAGR and test Sharpe both strictly better than the previous promoted run, and (2) test CAGR > 12%.** Scratch / ablation results are not promotion requirements. The Python promote path still has extra historical gates; do not add more. Align the code to these two conditions in a later change. |
...
5. **Intended ppo_discovery promotion has exactly two conditions; nothing else.** When the promote code is later aligned (do not expand gates in the meantime), a candidate may become `current` if and only if:
