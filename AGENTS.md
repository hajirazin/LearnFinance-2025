...
- **Temporal** is the outer orchestrator (replaced Prefect):
  ...
  - US PPO discovery workflow (`USPPODiscoveryAllocationWorkflow`): frozen current `halal_new` roster -> mandatory Alpaca/Benzinga news + OHLCV state (`/signals/ppo-discovery/state`) -> news-conditioned PPO inference (`/inference/ppo-discovery`) -> sell-wait-buy via the dedicated `ppo_discovery` Alpaca account (orders tagged `algorithm='ppo_discovery'`) -> AI summary + email. PPO training and evaluation always use the IBKR Singapore Tiered cost model with a fixed USD 10,000 historical capital base; Alpaca remains the execution broker, and experience labeling uses actual recorded account NAV. `run_id` is `paper:halal_new:YYYY-MM-DD`. PatchTST/LSTM are not inputs. Incomplete news aborts the week with zero orders. Training never auto-promotes.
  - US PPO discovery training workflow (`USPPODiscoveryTrainingWorkflow`): freeze `halal_new` snapshot (today's roster applied historically; survivorship bias disclosed) -> PPO weekly news ETL -> preflight -> two-stage train (seed 42 only) -> candidate artifact only -> training email. Manual `POST /train/ppo-discovery/promote` with `approved_by` is required to write `current`.
...
| `POST /train/ppo-discovery/promote` | Manual promotion to `current`. Gates (behavioral rule 5): (1) test CAGR and test Sharpe both strictly better than the previous promoted run, and (2) test CAGR > 12%. Scratch / ablation results are not promotion requirements. `approved_by` is a human audit field. |
...
4. **Production pipeline and experiments are different systems. Do not combine them.** `scratch/` (and any one-off research runner) is where hypotheses live: extra seeds, ablation *retrains*, “did the policy learn costs / news / the encoder?”, architecture probes, teacher policies, diagnostic-only comparisons, **trial configs**. The production path (`POST /train/*`, Temporal monthly/weekly jobs, `current`, live inference, orders) must stay the minimum that produces a candidate or a Monday allocation in a usable wall-clock. Do not add a full extra train, a required retrain ablation, or a promotion gate that exists only to answer a research question. Eval-only scoring of the already-trained policy is allowed when it is cheap; a second 10k-step PPO (or equivalent) inside `run_required_ablations` / `pipeline.py` is an experiment, not a train-job tax. If a result would change what we believe, run it under `scratch/` and leave the production job able to finish. Cautionary tale: wiring `no_transaction_cost_term` as a production retrain made every ppo_discovery train wait on a sixth full PPO and rendered the monthly job barely usable.

   **Graduation is the point of scratch, not mixing.** Try the idea only in `scratch/` first. If it succeeds (better test CAGR / Sharpe, or the specific hypothesis holds), **then** change production code to match — e.g. copy the winning `PPODiscoveryConfig` field, reward term, or architecture default into `brain_api/core/...` so the next real `POST /train/*` uses it. That is an explicit prod edit after the experiment, not “run the experiment inside the monthly job.” A failed scratch run stays in `scratch/`; do not leave forks, flags, or extra trains in the production pipeline “just in case.”

   **Scratch is never a model-promotion requirement.** Ablations, no-cost retrains, frozen-encoder runs, no-pretrain runs, equal-weight comparisons, Alpha-HRP pairing, news-shuffle, protocol-file hashes, and anything else that lives in `scratch/` or was added to answer a research question must not block `POST /train/ppo-discovery/promote` and must not be added to `evaluate_ppo_discovery_candidate` / `evaluate_ppo_discovery_promotion` as new failure reasons. They may appear on the training email as diagnostics only. Winning a scratch experiment does not auto-write `current`; it licenses a later production-code change, after which a normal train + the two numeric promote gates in rule 5 apply.

5. **ppo_discovery promotion has exactly two conditions; nothing else.** A candidate may become `current` if and only if:
   1. **Beats the previous promoted run on both test CAGR and test Sharpe** (strictly greater than the incumbent’s `test_cagr` and `test_sharpe`). If there is no incumbent, this comparison does not apply.
   2. **Test CAGR > 12%** (`PROMOTION_CAGR_FLOOR = 0.12`).
   Not promotion conditions: ablations (including `REQUIRED_ABLATIONS`), failed-seed completeness, Alpha-HRP CAGR or paired point estimate, unpaired evaluation acknowledgements, `repair_override`, `experiment_variant` beyond writing a full candidate, news-ablation bans, `code_revision` / `protocol_digest` matching, config-hash ceremonies, or any scratch metric. `approved_by` remains a human audit field on the manual POST; it is not a third numeric gate. SAC / LSTM / PatchTST keep their own existing health checks; this two-gate rule is ppo_discovery.
