"""HuggingFace Hub storage for SAC + PatchTST model artifacts.
...
from brain_api.core.config import get_hf_sac_patchtst_model_repo, get_hf_token
...
__all__ = ["HFModelInfo", "SACPatchTSTHuggingFaceModelStorage"]
...
class SACPatchTSTHuggingFaceModelStorage:
    """HuggingFace Hub storage for SAC + PatchTST model artifacts.
    ...
    def __init__(
        ...
        token: str | None = None,
        local_cache: SACPatchTSTLocalStorage | None = None,
    ):
        """Initialize SAC + PatchTST HuggingFace model storage.

        Args:
            repo_id: HuggingFace repo ID. Defaults to HF_SAC_PATCHTST_MODEL_REPO env var.
            token: HuggingFace API token.
            local_cache: Optional local storage for caching downloaded models.
        """
        self.repo_id = repo_id or get_hf_sac_patchtst_model_repo()
        self.token = token or get_hf_token()
        self.local_cache = local_cache or SACPatchTSTLocalStorage()
        self.api = HfApi(token=self.token)

        if not self.repo_id:
            raise ValueError(
                "HuggingFace SAC_PATCHTST model repo not configured. "
                "Set HF_SAC_PATCHTST_MODEL_REPO environment variable or pass repo_id."
            )
    ...
    def upload_model(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger.info(f"Uploading SAC_PatchTST model {version} to {self.repo_id}")
            ...
            self.api.upload_folder(
                ...
                repo_type="model",
                revision=version,
                commit_message=f"Add SAC_PatchTST model version {version}",
            )
            ...
            if make_current:
                ...
                self.api.upload_folder(
                    ...
                    repo_type="model",
                    revision="main",
                    commit_message=f"Promote SAC_PatchTST version {version} to current",
                )
    ...
    def download_model(
        ...
        Returns:
            SACPatchTSTArtifacts ready for inference
        """
        revision = version or "main"

        # Check local cache first
        if use_cache and version and self.local_cache.version_exists(version):
            logger.info(f"Loading SAC_PatchTST model {version} from local cache")
            return self.local_cache.load_artifacts(version)

        logger.info(
            f"Downloading SAC_PatchTST model from {self.repo_id} (revision: {revision})"
        )
        ...
        config = SACPatchTSTConfig.from_dict(config_dict)
        ...
        if use_cache and actual_version and actual_version != "main":
            logger.info(f"Caching SAC_PatchTST model {actual_version} locally")
            metadata_path = local_path / "metadata.json"
            metadata = {}
            if metadata_path.exists():
                with open(metadata_path) as f:
...
# LearnFinance SAC + PatchTST Model - {version}
...
- **Version**: {version}
- **Model Type**: SAC (Soft Actor-Critic) + PatchTST features
...
storage = SACPatchTSTHuggingFaceModelStorage(repo_id="{self.repo_id}")
