"""Local filesystem storage for SAC + PatchTST model artifacts.
...
@dataclass
class SACPatchTSTArtifacts:
    """Loaded SAC + PatchTST model artifacts for inference.
...
class SACPatchTSTLocalStorage:
    """Local filesystem storage for SAC + PatchTST model artifacts.

    Artifacts are stored under:
        {base_path}/models/sac_patchtst/{version}/
    ...
    The current version pointer is stored at:
        {base_path}/models/sac_patchtst/current
    """
    ...
    def __init__(self, base_path: Path | str | None = None):
        ...
        self._model_path = self.base_path / "models" / "sac_patchtst"
    ...
    def load_config(self, version: str) -> SACPatchTSTConfig:
        ...
        Returns:
            SACPatchTSTConfig reconstructed from config.json
        """
        config_path = self._version_path(version) / "config.json"
        with open(config_path) as f:
            config_dict = json.load(f)
        return SACPatchTSTConfig.from_dict(config_dict)
    ...
    def load_artifacts(self, version: str) -> SACPatchTSTArtifacts:
        ...
        Returns:
            SACPatchTSTArtifacts containing all loaded components.
        """
    ...
    def load_current_artifacts(self) -> SACPatchTSTArtifacts:
        """Load artifacts for the current version.

        Returns:
            SACPatchTSTArtifacts for current version.
        ...
        if version is None:
            raise ValueError("No current SAC PatchTST model version available")
        return self.load_artifacts(version)


def create_sac_patchtst_metadata(
    version: str,
    ...
    """Create metadata dictionary for SAC + PatchTST model.
