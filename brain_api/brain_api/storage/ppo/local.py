"""Local filesystem storage for PPO model artifacts (unified with dual forecasts).
...
@dataclass
class PPOArtifacts:
    """Loaded PPO model artifacts for inference.
...
class PPOLocalStorage:
    """Local filesystem storage for PPO model artifacts.

    Artifacts are stored under:
        {base_path}/models/ppo/{version}/
    ...
    def __init__(self, base_path: Path | str | None = None):
        ...
        self._model_path = self.base_path / "models" / "ppo"
    ...
    def load_config(self, version: str) -> PPOConfig:
        ...
        Returns:
            PPOConfig reconstructed from config.json
        """
    ...
    def load_current_artifacts(self) -> PPOArtifacts:
        """Load all artifacts for the current promoted version.

        Returns:
            PPOArtifacts containing everything needed for inference
        ...
        if version is None:
            raise ValueError(
                "No current PPO version set. "
                "Train a model first with POST /train/ppo/full"
            )
...
def create_ppo_metadata(
    ...
    """Create metadata dict for PPO training.
