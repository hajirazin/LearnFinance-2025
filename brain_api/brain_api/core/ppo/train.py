"""PPO full training implementation with dual forecasts (LSTM + PatchTST).

Contains the main train_ppo function and related utilities.
"""
...
from brain_api.core.ppo.data import TrainingData
from brain_api.core.ppo.model import PPOActorCritic
from brain_api.core.ppo.trainer import PPOTrainer
...
def train_ppo(
    ...
    """Train PPO portfolio allocator with dual forecasts.
    ...
    train_lstm = training_data.lstm_forecasts[:split_idx]
    train_patchtst = training_data.patchtst_forecasts[:split_idx]

    val_returns = training_data.symbol_returns[split_idx:]
    val_signals = training_data.signals[split_idx:]
    val_lstm = training_data.lstm_forecasts[split_idx:]
    val_patchtst = training_data.patchtst_forecasts[split_idx:]
    ...
    train_env = PortfolioEnv(
        ...
        lstm_forecasts=train_lstm,
        patchtst_forecasts=train_patchtst,
    ...
    val_env = PortfolioEnv(
        ...
        signals=val_signals,
        lstm_forecasts=val_lstm,
        patchtst_forecasts=val_patchtst,
        symbol_order=training_data.symbol_order,
        config=config,
    )
    ...
    print(
        f"[PPO] Final metrics: policy_loss={final_policy_loss:.4f}, value_loss={final_value_loss:.4f}"
    )
    print(
        f"[PPO] Eval: sharpe={eval_metrics['sharpe']:.4f}, cagr={eval_metrics['cagr'] * 100:.2f}%, max_dd={eval_metrics['max_drawdown'] * 100:.2f}%"
    )
