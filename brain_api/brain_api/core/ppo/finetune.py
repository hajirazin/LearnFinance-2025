...
from brain_api.core.ppo.train import PPOTrainingResult, evaluate_policy
...
def finetune_ppo(
    ...
    prior_config: PPOConfig,
    finetune_config: PPOFinetuneConfig | None = None,
) -> PPOTrainingResult:
    """Fine-tune PPO on recent data.
    ...
    train_lstm = training_data.lstm_forecasts[-n_weeks:-val_weeks]
    train_patchtst = training_data.patchtst_forecasts[-n_weeks:-val_weeks]

    val_returns = training_data.symbol_returns[-val_weeks:]
    val_signals = training_data.signals[-val_weeks:]
    val_lstm = training_data.lstm_forecasts[-val_weeks:]
    val_patchtst = training_data.patchtst_forecasts[-val_weeks:]
    ...
    finetune_ppo_config = PPOBaseConfig(
        hidden_sizes=prior_config.hidden_sizes,
        activation=prior_config.activation,
