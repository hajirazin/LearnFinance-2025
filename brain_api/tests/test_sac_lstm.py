"""API-level tests for SAC + LSTM training and inference endpoints.
...
from brain_api.core.sac_lstm.training import TrainingData
...
from brain_api.routes.inference import get_sac_lstm_storage as get_inference_storage
from brain_api.storage.sac_lstm import SACLSTMLocalStorage, create_sac_lstm_metadata
...
def mock_config() -> SACLSTMConfig:
    """Return a minimal config for fast testing."""
    return SACLSTMConfig(
        n_stocks=5,
...
def create_mock_training_result(config: SACLSTMConfig) -> SACLSTMTrainingResult:
...
def temp_storage():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        yield SACLSTMLocalStorage(base_path=tmpdir)
...
def trained_model_storage():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        storage = SACLSTMLocalStorage(base_path=tmpdir)
        config = mock_config()
...
class TestSACLSTMInference:
    """Tests for /inference/sac_lstm endpoint."""
...
class TestSACLSTMFinetune:
    ...
    def test_finetune_without_prior_returns_400(self, temp_storage):
        ...
        app.dependency_overrides.clear()
        app.dependency_overrides[get_sac_lstm_storage] = lambda: temp_storage
        ...
        assert "No prior SAC_LSTM model" in response.json()["detail"]
...
class TestSACLSTMStorage:
    """Tests for SAC LSTM storage functionality."""
